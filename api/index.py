...
logger = logging.getLogger("fr3oon-helper-bot")
...
def tg_api(method, payload=None):
    ...
    try:
        ...
        data = response.json()
        if not data.get("ok"):
            logger.warning("Telegram API %s failed: %s", method, data.get("description", "unknown error"))
        return data
...
def log_event(tag, **fields):
    safe_fields = {k: v for k, v in fields.items() if k not in ("token", "api_key", "BOT_TOKEN", "OCR_SPACE_API_KEY")}
    logger.info("%s %s", tag, safe_fields)
...
def get_bot_user_id():
    ...
    result = tg_api("getMe")
    try:
        bot_user_id = int((result.get("result") or {}).get("id") or 0)
    except Exception:
        bot_user_id = 0
    return bot_user_id


def bot_permissions(chat_id):
    bot_id = get_bot_user_id()
    if not bot_id:
        return {}
    result = tg_api("getChatMember", {"chat_id": chat_id, "user_id": bot_id})
    member = result.get("result") or {}
    return member if member.get("status") in ("administrator", "creator") else {}


def bot_can_delete(chat_id):
    member = bot_permissions(chat_id)
    return member.get("status") == "creator" or bool(member.get("can_delete_messages"))


def bot_can_restrict(chat_id):
    member = bot_permissions(chat_id)
    return member.get("status") == "creator" or bool(member.get("can_restrict_members"))


def require_bot_permissions(chat_id, delete=False, restrict=False):
    missing = []
    if delete and not bot_can_delete(chat_id):
        missing.append("Delete Messages")
    if restrict and not bot_can_restrict(chat_id):
        missing.append("Ban Users")
    if missing:
        return False, "البوت محتاج صلاحية: " + " و ".join(missing)
    return True, ""
...
def caption_has_card_hint(msg):
    text = get_text(msg).translate(ARABIC_DIGITS_MAP)
    if not text:
        return False
    normalized = normalize_arabic(text)
    has_code_word = any(word in normalized for word in ("رمزالاستبدال", "رمزالاسترداد", "كود", "code", "redeemcode"))
    has_date = bool(re.search(r"\b20\d{2}[./-]\d{1,2}[./-]\d{1,2}\b", text))
    has_nine_digits = bool(re.search(r"(?<!\d)\d{8,10}(?!\d)", text))
    has_pubg_words = "انا" in normalized and "اللاعب" in normalized
    return (has_code_word and (has_nine_digits or has_date)) or (has_nine_digits and has_pubg_words)


def image_has_pubg_card_shape(image_bytes):
    try:
    ...
    ratio = h / max(w, 1)
    if ratio < 0.75 or ratio > 2.7:
        return False

    sample = img.resize((80, 80))
    pixels = list(sample.getdata())
    total = max(len(pixels), 1)
    dark_blue_gray = sum(1 for r, g, b in pixels if b >= r + 8 and b >= g - 8 and 18 <= r <= 95 and 25 <= g <= 115 and 45 <= b <= 145)
    dark_ratio = dark_blue_gray / total

    code_band = img.crop((int(w * 0.30), int(h * 0.55), int(w * 0.70), int(h * 0.76))).resize((80, 36))
    band_pixels = list(code_band.getdata())
    orange = sum(
        1 for r, g, b in band_pixels
        if r >= 115 and 35 <= g <= 190 and b <= 145 and r >= g + 20 and g >= b
    ) / max(len(band_pixels), 1)

    gray = ImageOps.grayscale(img.crop((int(w * 0.18), int(h * 0.42), int(w * 0.82), int(h * 0.85))).resize((120, 80)))
    hist = gray.histogram()
    bright = sum(hist[185:]) / max(sum(hist), 1)
    mid = sum(hist[70:185]) / max(sum(hist), 1)
...
def should_try_card_ocr(msg, image_bytes):
    if caption_has_card_hint(msg):
        return True
    if not image_bytes:
        return False
    return image_has_pubg_card_shape(image_bytes)
...
def parse_card_info_from_photo(msg, user):
    image_bytes = get_photo_bytes(msg)
    if not image_bytes:
        return None
    return parse_card_info_from_photo_bytes(image_bytes, msg, user)


def parse_card_info_from_photo_bytes(image_bytes, msg, user):
    if not image_bytes:
        return None
...
def parse_card_info_from_ocr_texts(texts, msg, user):
    if not texts:
        return None
    code = extract_code_from_ocr_results(texts)
    if not code:
...
def send_photo_ocr_help(chat_id, reply_to_message_id=None):
    return send_auto_delete_message(
        chat_id,
        "❌ معرفتش أقرأ رمز الاستبدال. ارفع الصورة كاملة وبجودة أوضح.",
        reply_to_message_id,
        20,
    )
...
def send_auto_delete_message(chat_id, text, reply_to_message_id=None, seconds=30, reply_markup=None):
    ...
    try:
        sent_id = (result.get("result") or {}).get("message_id")
        if result.get("ok") and sent_id and seconds and seconds > 0:
            timer = threading.Timer(seconds, delete_message, args=(chat_id, sent_id))
            timer.daemon = True
            timer.start()
    except Exception:
        logger.exception("Failed to schedule temporary message cleanup")
    return result
...
def moderation_panel_keyboard(group_chat_id, target_user_id, target_message_id, requester_id, message_url=None):
    rows = [
        [
            {"text": "⚠️ إنذار", "callback_data": f"p:w:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
            {"text": "🔇 كتم 10 د", "callback_data": f"p:m10:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
        ],
        [
            {"text": "🔇 كتم 30 د", "callback_data": f"p:m30:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
            {"text": "🔇 كتم ساعة", "callback_data": f"p:m60:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
        ],
        [
            {"text": "🚫 حظر نهائي", "callback_data": f"p:b:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
            {"text": "✅ فك الكتم", "callback_data": f"p:u:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
        ],
        [
            {"text": "♻️ تصفير الإنذارات", "callback_data": f"p:r:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
            {"text": "🗑 حذف الرسالة الأصلية", "callback_data": f"p:d:{group_chat_id}:{target_user_id}:{target_message_id}:{requester_id}"},
        ],
    ]
    if message_url:
...
def target_username_text(user):
    username = (user.get("username") or "").strip()
    return f"@{username}" if username else "لا يوجد"


def moderation_panel_text(chat, target_user, target_message, reporter=None):
    target_id = target_user.get("id")
    lines = [
        "👮 <b>لوحة إدارة خاصة</b>",
        "",
        f"العضو: <b>{esc(sender_display_name(target_user))}</b>",
        f"Username: <b>{esc(target_username_text(target_user))}</b>",
        f"User ID: <code>{target_id}</code>",
        f"الجروب: <b>{esc(chat.get('title') or 'الجروب')}</b>",
        f"الإنذارات: <b>{warning_count(chat.get('id'), target_id)}</b> من 3",
        f"الرسالة: <code>{message_preview(target_message)}</code>",
    ]
    if reporter:
        lines.insert(2, f"المبلغ: <b>{esc(sender_display_name(reporter))}</b>")
    return "\n".join(lines)


def send_private_mod_panel(admin_id, chat, target_user, target_message, requester_id=None, reporter=None):
    requester_id = requester_id or admin_id
    target_id = target_user.get("id")
    target_message_id = target_message.get("message_id")
    return tg_api("sendMessage", {
        "chat_id": admin_id,
        "text": moderation_panel_text(chat, target_user, target_message, reporter),
        "parse_mode": "HTML",
...
def handle_new_members(msg):
    ...
    for member in members:
        ...
        if REQUIRE_RULES_ACCEPTANCE and not user_is_admin(chat_id, user_id):
        ...
        send_auto_delete_message(
            chat_id,
            "أهلًا بك في جروب تبادل بطاقات PUBG 👋\n"
            "✅ التبادل وصور البطاقات مسموحة\n"
            "❌ البيع والشراء والروابط وطلب بيانات الحساب ممنوع\n"
            "⚠️ النصب أو السبام يعرضك للحظر",
            None,
...
def handle_group_command(msg, is_admin):
    ...
    if command == "/report":
        ...
        if not ADMIN_USER_ID:
            send_auto_delete_message(chat_id, "الأدمن يضيف ADMIN_USER_ID في إعدادات Vercel.", replied.get("message_id"), WARNING_DELETE_SECONDS)
            return True
        private_result = send_private_mod_panel(ADMIN_USER_ID, chat, reported_user, replied, ADMIN_USER_ID, user)
        if private_result.get("ok"):
            send_auto_delete_message(chat_id, "✅ تم إرسال البلاغ للأدمن.", replied.get("message_id"), WARNING_DELETE_SECONDS)
        else:
            send_auto_delete_message(chat_id, "افتح خاص البوت واضغط Start أولًا.", replied.get("message_id"), WARNING_DELETE_SECONDS)
        return True
    if command in ("/mod", "/panel"):
        ...
        private_result = send_private_mod_panel(user_id, chat, target_user, msg.get("reply_to_message") or {}, user_id)

        delete_message(chat_id, message_id)
        if not private_result.get("ok"):
            send_auto_delete_message(chat_id, "افتح خاص البوت واضغط Start أولًا.", message_id, WARNING_DELETE_SECONDS)
        return True
    ...
    if command in ("/warn", "/mute", "/unmute", "/ban", "/unban", "/resetwarn", "/del"):
        ...
        if not target_id:
            ...
            return True
        needs_delete = command == "/del"
        needs_restrict = command in ("/warn", "/mute", "/unmute", "/ban", "/unban")
        ok, permission_reason = require_bot_permissions(chat_id, delete=needs_delete, restrict=needs_restrict)
        if not ok:
            send_simple_notice(chat_id, message_id, permission_reason)
            return True
        allowed, reason = target_can_be_moderated(chat_id, target_id)
...
def ocr_text_has_card_hint(texts):
    joined = "\n".join(text for _, text in (texts or [])).translate(ARABIC_DIGITS_MAP)
    normalized = normalize_arabic(joined)
    has_code_word = any(word in normalized for word in ("رمزالاستبدال", "رمزالاسترداد", "code", "redeemcode"))
    has_date = bool(re.search(r"\b20\d{2}[./-]\d{1,2}[./-]\d{1,2}\b", joined))
    has_nine_digits = bool(re.search(r"(?<!\d)\d{8,10}(?!\d)", joined))
    has_pubg_words = "انا" in normalized and "اللاعب" in normalized
    return (has_code_word and (has_date or has_nine_digits)) or (has_date and has_nine_digits) or has_pubg_words


def handle_photo_message(msg, chat, user):
    chat_id = chat.get("id")
    message_id = msg.get("message_id")
    user_id = user.get("id")
    log_event("[PHOTO_RECEIVED]", chat_id=chat_id, user_id=user_id, message_id=message_id, media_group_id=msg.get("media_group_id"))

    info = None
    image_bytes = None
    ocr_texts = []
    looks_like_card_after_read = caption_has_card_hint(msg)

    if OCR_FROM_IMAGE:
        log_event("[PHOTO_PROCESSING_STARTED]", chat_id=chat_id, user_id=user_id, message_id=message_id)
        image_bytes = get_photo_bytes(msg)
        if not image_bytes:
            log_event("[PHOTO_IGNORED_REASON]", reason="telegram_file_download_failed", chat_id=chat_id, message_id=message_id)
        else:
            looks_like_card_after_read = looks_like_card_after_read or image_has_pubg_card_shape(image_bytes)
            try:
                log_event("[OCR_STARTED]", chat_id=chat_id, user_id=user_id, message_id=message_id)
                ocr_texts = run_ocr_on_image_bytes(image_bytes)
                log_event("[OCR_RESULT]", text_blocks=len(ocr_texts), chat_id=chat_id, message_id=message_id)
                info = parse_card_info_from_ocr_texts(ocr_texts, msg, user)
                looks_like_card_after_read = looks_like_card_after_read or ocr_text_has_card_hint(ocr_texts)
            except Exception as exc:
                log_event("[OCR_ERROR]", error=type(exc).__name__, chat_id=chat_id, message_id=message_id)
                logger.exception("OCR failed for a Telegram photo")
    ...
    if info:
        code = info.get("code")
        log_event("[CODE_FOUND]", chat_id=chat_id, user_id=user_id, message_id=message_id, code_length=len(str(code or "")))
        expiry_at = info.get("expiry_at")
    ...
    if looks_like_card_after_read:
        stats["ocr_failed"] += 1
        log_event("[PHOTO_IGNORED_REASON]", reason="card_hint_without_code", chat_id=chat_id, message_id=message_id)
        send_photo_ocr_help(chat_id, message_id)
        return True

    log_event("[PHOTO_IGNORED_REASON]", reason="ordinary_photo_or_ad", chat_id=chat_id, message_id=message_id)
    if REQUIRE_PHOTO_TEMPLATE and not user_is_admin(chat_id, user_id):
        delete_message(chat_id, message_id)
        send_auto_delete_message(chat_id, "مسموح بصور البطاقات فقط.", None, WARNING_DELETE_SECONDS)
        return True
    return True
...
def handle_callback_query(query):
    ...
    if data.startswith("rules:"):
        ...
        tg_api("editMessageText", {
            ...
            "text": "✅ تم قبول القوانين، يمكنك المشاركة الآن.",
            "parse_mode": "HTML",
        })
        tg_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": "تم تفعيل مشاركتك ✅"})
        return

    if data.startswith("pmod:") or data.startswith("p:"):
        if not callback_id or not clicker_id:
            return
        try:
            parts = data.split(":")
            if parts[0] == "pmod":
                _, action, group_chat_id_raw, target_id_raw, target_message_id_raw = parts[:5]
                requester_id = clicker_id
            else:
                _, action, group_chat_id_raw, target_id_raw, target_message_id_raw, requester_id_raw = parts[:6]
        ...
        if clicker_id != requester_id:
            tg_api("answerCallbackQuery", {
                ...
                "text": "غير مسموح لك باستخدام لوحة الإدارة.",
                "show_alert": True,
            })
            return
        ...
        if action == "w":
            ok, reason = require_bot_permissions(group_chat_id, restrict=True)
            if not ok:
                tg_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": reason, "show_alert": True})
                return
            ...
            if not ok:
                tg_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": reason, "show_alert": True})
                return
            minutes = int(action[1:] or DEFAULT_MUTE_MINUTES) if action[1:].isdigit() else DEFAULT_MUTE_MINUTES
            until_date = int(time.time()) + minutes * 60
            set_member_restricted(group_chat_id, target_id, True, until_date)
            admin_action_notice(group_chat_id, target_message_id, f"🔇 تم كتم العضو لمدة {minutes} دقيقة.")
            result_text = f"🔇 تم الكتم {minutes} دقيقة"
        elif action == "b":
            ok, reason = require_bot_permissions(group_chat_id, restrict=True)
            if not ok:
                tg_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": reason, "show_alert": True})
            ...
            ok, reason = require_bot_permissions(group_chat_id, delete=True)
            if not ok:
                tg_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": reason, "show_alert": True})
                return
...
def handle_message(msg):
    ...
    # الصور تتعالج قبل فلاتر المخالفات حتى لا تمنع بطاقة سليمة من الوصول للـ OCR.
    if msg.get("photo"):
        handle_photo_message(msg, chat, user)
        return
