...
def handle_group_command(msg, is_admin):
    ...
    if command == "/help":
        send_simple_notice(chat_id, message_id, (
            "🤖 <b>مساعدة الجروب</b>\n\n"
            "/rules — القوانين\n"
            "/admin — استدعاء الأدمن\n"
            "/report — رد على رسالة للإبلاغ عنها\n"
            "/mywarnings — عدد مخالفاتك"
        ))
        return True
    if command == "/admin":
        send_simple_notice(chat_id, message_id, admin_list_text(chat_id))
        return True
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(200, {
            "ok": True,
            "message": "Fr3oon Helper Bot V18 group manager is running",
            "rules": "V11 stable card OCR + welcome/rules/moderation/warnings/report/slow/lock/FAQ tools",
        })
