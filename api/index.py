...
def send_auto_delete_message(chat_id, text, reply_to_message_id=None, seconds=30, reply_markup=None):
    ...
    try:
        ...
        if result.get("ok") and sent_id and seconds and seconds > 0:
            threading.Timer(seconds, delete_message, args=(chat_id, sent_id)).start()
    except Exception:
...
def handle_message(msg):
    ...
    if msg.get("photo"):
        info = None
        image_bytes = get_photo_bytes(msg) if OCR_FROM_IMAGE else None
        looks_like_card = should_try_card_ocr(msg, image_bytes)
        if OCR_FROM_IMAGE and looks_like_card:
            info = parse_card_info_from_photo_bytes(image_bytes, msg, user)
        if not info:
        ...
        if looks_like_card:
            ...
            return
        if REQUIRE_PHOTO_TEMPLATE and not is_admin:
            delete_message(chat_id, message_id)
