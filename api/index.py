...
def build_cloud_crops(image_bytes):
    ...
    # سطر رمز الاستبدال في تصميم بطاقات ببجي يقع غالبًا في النصف السفلي.
    code_wide = img.crop((int(w * 0.20), int(h * 0.56), int(w * 0.82), int(h * 0.76)))
    code_wide = code_wide.resize((max(code_wide.width * 3, 1), max(code_wide.height * 3, 1)))
    crops.append(("code-wide.png", image_to_png_bytes(code_wide)))

    code_tight = img.crop((int(w * 0.30), int(h * 0.60), int(w * 0.72), int(h * 0.72)))
    code_tight = ImageEnhance.Contrast(code_tight.resize((max(code_tight.width * 4, 1), max(code_tight.height * 4, 1)))).enhance(2.0)
    crops.append(("code-tight.png", image_to_png_bytes(code_tight)))
    return crops
...
def run_ocr_on_image_bytes(image_bytes):
    ...
    if OCR_PREFER_CLOUD:
        # الصورة كاملة + قصّتان مخصصتان لسطر الكود.
        texts.extend(run_ocr_space(image_bytes, "card-full.png", "2"))
        for filename, crop_bytes in build_cloud_crops(image_bytes):
            texts.extend(run_ocr_space(crop_bytes, filename, "2"))
            # المحرك الأول أحيانًا يكون أدق مع الأرقام البرتقالية.
            texts.extend(run_ocr_space(crop_bytes, filename, "1"))
...
def extract_code_from_text(text):
    ...
    # أكواد هذه البطاقات رقمية من 9 خانات. لا نرسل نتيجة قصيرة أو بها حروف حتى لا ننشر كودًا غلطًا.
    votes = {}

    def add_candidate(token, weight):
        normalized = normalize_ocr_token(token)
        if not normalized or not normalized.isdigit() or len(normalized) != 9:
            return
        # استبعاد شكل تاريخ YYYYMMDD مع رقم زائد، إن ظهر بالخطأ.
        if normalized.startswith("2026") or normalized.startswith("2027") or normalized.startswith("2028"):
            return
        votes[normalized] = votes.get(normalized, 0) + weight

    for pattern in CODE_HINT_PATTERNS:
        for match in pattern.finditer(raw):
            add_candidate(match.group(1), 10)

    for match in re.finditer(r"\b([A-Z0-9]{8,11})\b\s*[:：]", raw):
        add_candidate(match.group(1), 7)

    for match in re.finditer(r"\b[A-Z0-9]{8,11}\b", raw):
        add_candidate(match.group(0), 2)
...
def extract_issue_datetime_from_text(texts):
    ...
    for text in texts:
        ...
        for m in re.finditer(r"(20\d{2})[./-](\d{1,2})[./-](\d{1,2})(?:\s+(\d{1,2}[:.]\d{2}(?::\d{2})?))?", raw):
            ...
            if dt:
                dated.append((2 if m.group(4) else 1, dt))
    if not dated:
        return None
