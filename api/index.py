...
def moderation_panel_keyboard(target_user_id, target_message_id):
    return {
        "inline_keyboard": [
            [
                {"text": "⚠️ إنذار", "callback_data": f"mod:w:{target_user_id}:{target_message_id}"},
                {"text": f"🔇 كتم {DEFAULT_MUTE_MINUTES}د", "callback_data": f"mod:m:{target_user_id}:{target_message_id}"},
            ],
            [
                {"text": "🚫 حظر فوري", "callback_data": f"mod:b:{target_user_id}:{target_message_id}"},
                {"text": "🗑 حذف الرسالة", "callback_data": f"mod:d:{target_user_id}:{target_message_id}"},
            ],
            [
                {"text": "✅ فك الكتم", "callback_data": f"mod:u:{target_user_id}:{target_message_id}"},
                {"text": "♻️ تصفير الإنذارات", "callback_data": f"mod:r:{target_user_id}:{target_message_id}"},
            ],
        ]
    }
...
def handle_group_command(msg, is_admin):
    ...
    if command in ("/mod", "/panel"):
        ...
        tg_api("sendMessage", {
            ...
            "text": f"👮 <b>لوحة إدارة العضو:</b> {sender_display_name(target_user)}\nاختر الإجراء المطلوب:",
            "parse_mode": "HTML",
            "reply_markup": moderation_panel_keyboard(target_id, target_message_id),
        })
        return True
...
def handle_callback_query(query):
    ...
    if data.startswith("mod:"):
        if not callback_id or not chat_id or not clicker_id:
            return
        if not user_is_admin(chat_id, clicker_id):
            tg_api("answerCallbackQuery", {
                "callback_query_id": callback_id,
                "text": "الأزرار دي للأدمن فقط",
                "show_alert": True,
            })
            return
        try:
            _, action, target_id_raw, target_message_id_raw = data.split(":", 3)
        ...
        if action == "w":
            add_violation(chat_id, target_id, "إنذار يدوي من الأدمن", target_message_id)
            result_text = "⚠️ تم إعطاء العضو إنذارًا"
        elif action == "m":
            until_date = int(time.time()) + DEFAULT_MUTE_MINUTES * 60
            set_member_restricted(chat_id, target_id, True, until_date)
            admin_action_notice(chat_id, target_message_id, f"🔇 تم كتم العضو لمدة {DEFAULT_MUTE_MINUTES} دقائق.")
            result_text = f"🔇 تم الكتم {DEFAULT_MUTE_MINUTES} دقائق"
        elif action == "b":
            tg_api("banChatMember", {"chat_id": chat_id, "user_id": target_id})
            admin_action_notice(chat_id, target_message_id, "🚫 تم حظر العضو فورًا بواسطة الأدمن.")
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(200, {
            "ok": True,
            "message": "Fr3oon Helper Bot V19 admin control is running",
            "rules": "V11 stable card OCR + V19 admin moderation panel + contact button + group tools",
        })
