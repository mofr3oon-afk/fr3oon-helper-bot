...
def run_ocr_space(image_bytes, filename="card.png", engine="2"):
    ...
    try:
        ...
        data = response.json()
        print("[OCR_SPACE_STATUS]", response.status_code, flush=True)
        print("[OCR_SPACE_RESPONSE]", json.dumps(data, ensure_ascii=False)[:12000], flush=True)
        results = []
        ...
        print("[OCR_SPACE_EXCEPTION]", repr(e), flush=True)
        return []
...
def run_ocr_on_image_bytes(image_bytes):
    """Read both the full screenshot and tightly cropped code bands.

    One full-image OCR request gives context/date. Dynamic orange crops make
    the 9-digit redemption code much larger, which fixes the multi-card layout.
    """
    texts = []

    if OCR_SPACE_API_KEY:
        # One full-image request for context/date.
        for text in run_ocr_space(image_bytes, "card-full.png", "2"):
            texts.append(("full_e2", text))

        # Up to two dynamic crops: normal crop + orange-isolated crop.
        # This stays within a small number of API calls but greatly improves code OCR.
        for idx, (filename, crop_bytes) in enumerate(build_cloud_crops(image_bytes)[:2], start=1):
            for text in run_ocr_space(crop_bytes, filename, "2"):
                texts.append((f"code_crop_{idx}", text))

    # Local OCR fallback where the binary exists.
    try:
        img = Image.open(BytesIO(image_bytes)).convert("RGB")
        for idx, (filename, crop_bytes) in enumerate(build_cloud_crops(image_bytes)[:2], start=1):
            try:
                crop_img = Image.open(BytesIO(crop_bytes)).convert("L")
                ...
                if txt and txt.strip():
                    texts.append((f"local_crop_{idx}", txt.strip()))
            except Exception:
                pass
    except Exception:
        pass
...
def extract_code_from_ocr_results(results):
    """Extract a 9-digit code, strongly preferring the dedicated code crops."""
    if not results:
        print("[OCR_RESULTS] EMPTY", flush=True)
        return None

    print("[OCR_RESULTS_COUNT]", len(results), flush=True)
    counts = {}

    def add(code, weight, source):
        if not code or not code.isdigit() or len(code) != 9:
            return
        if code.startswith(("2026", "2027", "2028", "2029", "2030")):
            return
        if len(set(code)) <= 2:
            return
        counts[code] = counts.get(code, 0) + weight
        print("[OCR_CANDIDATE]", source, code, "+", weight, flush=True)

    for source, text in results:
        print(f"[OCR_TEXT:{source}]", repr(text)[:6000], flush=True)
        raw = text.translate(ARABIC_DIGITS_MAP).upper()
        is_crop = "crop" in source
        weight = 8 if is_crop else 2

        # Exact alphanumeric OCR token (allows common letter/digit confusions).
        for token in re.findall(r"(?<![A-Z0-9])[A-Z0-9]{9}(?![A-Z0-9])", raw):
            normalized = normalize_ocr_token(token)
            add(normalized, weight, source)

        # OCR may insert spaces, dots or hyphens between the nine digits.
        for m in re.finditer(r"(?<!\d)([0-9OQDISGTLBZ](?:[\s._:/-]*[0-9OQDISGTLBZ]){8})(?!\d)", raw):
            normalized = normalize_ocr_token(m.group(1))
            add(normalized, weight + (4 if is_crop else 0), source)

        # On a dedicated crop, accept the only nine-digit sequence after removing noise.
        if is_crop:
            compact = normalize_ocr_token(raw)
            digit_runs = re.findall(r"\d{9}", compact)
            if len(set(digit_runs)) == 1:
                add(digit_runs[0], weight + 5, source)

    print("[OCR_CODE_CANDIDATES]", counts, flush=True)
    if not counts:
        print("[OCR_SELECTED_CODE] NONE", flush=True)
        return None

    ranked = sorted(counts.items(), key=lambda item: item[1], reverse=True)
    best_code, best_score = ranked[0]
    if len(ranked) > 1 and ranked[1][1] == best_score:
        print("[OCR_SELECTED_CODE] TIE", ranked[:5], flush=True)
        return None
    print("[OCR_SELECTED_CODE]", best_code, "score=", best_score, flush=True)
    return best_code
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(200, {
            "ok": True,
            "message": "Fr3oon Helper Bot V17 crop-assisted OCR is running",
