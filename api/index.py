...
def run_ocr_on_image_bytes(image_bytes):
    ...
    # Keep cloud usage low: one full-image call + at most two code-crop calls.
    # The previous version made many calls per photo and could hit free API limits.
    if OCR_PREFER_CLOUD:
        for text in run_ocr_space(image_bytes, "card-full.png", "2"):
            texts.append(("full", text))
        for filename, crop_bytes in build_cloud_crops(image_bytes)[:2]:
    ...
    # Local fallback. It costs no OCR API quota.
...
def extract_code_from_ocr_results(results):
    ...
    for source, text in results:
        # الصورة الكاملة تُستخدم للتاريخ فقط حتى لا نلتقط أرقامًا أخرى.
        ...
        for token in re.findall(r"[A-Z0-9]{7,13}", raw):
            ...
            # أي رقم 9 خانات من قصاصة الكود نفسها يُعد نتيجة قوية.
            is_code_crop = source.startswith("code-") or "orange" in source or "dynamic" in source or source == "code_psm7"
            weight = 6 if "orange" in source else (5 if is_code_crop else 2)
            votes[normalized] = votes.get(normalized, 0) + weight
            sources_by_code.setdefault(normalized, set()).add(source)
            if is_code_crop:
                crop_candidates.append(normalized)
    ...
    # لو قصاصة الكود رجعت رقم 9 خانات وحيد، نقبله حتى لو ظهر من محرك واحد.
    unique_crop_codes = set(crop_candidates)
    if len(unique_crop_codes) == 1:
        return next(iter(unique_crop_codes))

    # لو أكثر من نتيجة، لا نقبل إلا الأفضل بفارق واضح.
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(200, {
            "ok": True,
            "message": "Fr3oon Helper Bot V14 reliable OCR is running",
